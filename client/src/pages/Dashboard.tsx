...
export const Dashboard: React.FC = () => {
  ...
  const sensorList = [
    ...
    { id: 'temperature', titleKey: 'sensors.motorTemp', unit: t('sensors.celsius'), icon: Thermometer, max: 100, thresholds: { warning: 60, critical: 70 }, gaugeType: 'thermometer' as const },
    ...
    >
      ...
      >
        <div>
          ...
          <p className="text-gray-400 text-sm font-mono mt-1">{t('dashboard.sessionId')}: <span className="text-primary">{data.timestamp}</span></p>
        </div>
        <div className="flex items-center gap-6">
           <div className="flex items-center gap-3 bg-white/5 px-4 py-2 rounded-full border border-white/10">
              <span className="text-xs font-bold text-gray-400 uppercase tracking-wider">{t('dashboard.systemStatus')}</span>
